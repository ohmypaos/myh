...
import { LOT, RACK, heightsOf } from './lot.js';
import { levels, openingFloor, underStairsOf, isUnderStair, stairCabinetOf, stepsOf, overhangsOf, lightRoofs, roofOver, dropCeilingsOf, postsOf, roofInsulationOf,
...
export const CHECKS = [
  ...
  'Bậc, mái hiên, diềm mái hiên, tường nâng, cốt sàn riêng, lớp chống nóng mái khai đúng chỗ; bậc nằm gọn trong sân, không vướng nội thất hay cánh cửa',
...
export function validate(v){
  ...
  const L=leftRooms.filter(r=>!isUnderStair(v,r)).reduce((s,r)=>s+A(r),0);  // phòng gầm thang lồng trong buồng thang
  ...
  const kin=v.rooms.filter(r=>r[6]!=='yard'&&r[0]!=='R3'&&!isUnderStair(v,r)).reduce((s,r)=>s+A(r),0);
  if(Math.abs(kin-v.areas.kin)>.01) e.push(`DT kín khai báo ${v.areas.kin} ≠ tính ra ${n(kin)}`);

  const ys=[...new Set(v.rooms.filter(r=>r[0][0]==='L'&&r[4]>=2.4&&!isUnderStair(v,r))
  ...
  // Cửa 2 cánh quét hai ô vuông nửa lỗ, mỗi ô một cánh. `sill` là đáy cánh — cốt ngưỡng phía cao.
  const levelsV = levels(heightsOf(v));
  const swings = v.doors.filter(d=>d[5]==='swing'||d[5]==='double').flatMap(([id,ax,pos,a,b,kind,,open])=>{
    const sill = openingFloor(v, levelsV, ax, pos, a, b);
    const zone=(a,b)=>{
      const Ln=b-a;
      return ax==='v'
    ...
    return (kind==='double' ? [zone(a,(a+b)/2), zone((a+b)/2,b)] : [zone(a,b)]).map(z=>({...z, sill}));
  });
  ...
  for(const f of fasciasOf(v)) if(f.error) e.push(f.error);
  ...
  for(const s of stepsOf(v)){
    ...
    /* Cánh mở ra phía bậc thì lơ lửng trên bậc: bậc sát cửa thấp hơn ngưỡng một nấc, đáy cánh ở cốt ngưỡng
       (chủ nhà chốt 14/9/2026 cho D2). Chỉ báo khi đáy cánh không cao hơn mặt bậc cao nhất — cửa ở cốt khác
       quét sang bậc cao hơn đáy cánh thì đụng thật. */
    const stepTop = s.top - s.rise;
    for(const z of swings)
      if(ovl(x,x+w,z.x0,z.x1)>0.02 && ovl(y,y+h,z.y0,z.y1)>0.02 && z.sill <= stepTop + 0.01)
  ...
  const nests = underStairsOf(v);
  for(const u of nests) if(u.error) e.push(u.error);
  /* Tủ module dưới gầm thang: cao suy ra theo mặt dưới bậc — phải thật sự nằm dưới thang và đủ cao để làm tủ. */
  for(const f of v.furn) if(f[0]==='stairCab'){ const c = stairCabinetOf(v, f); if(c.error) e.push(c.error); }
  for(const s of stairsOf(v)){
    if(s.error){ e.push(s.error); continue; }
    e.push(...s.errors);
    const {x,y,w,h} = s.hole;
    /* Nội thất và cánh cửa nằm trọn trong phòng gầm thang ở **dưới** bậc, không chồng bậc — khoảng đầu check-3d soi. */
    const under = nests.filter(u=>!u.error&&u.stair===s.id).map(u=>u.rect);
    const inNest = (x0,x1,y0,y1) => under.some(r=>x0>=r.x-.01&&x1<=r.x+r.w+.01&&y0>=r.y-.01&&y1<=r.y+r.h+.01);
    for(const [k,fx,fy,fw,fh] of v.furn)
      if(ovl(x,x+w,fx,fx+fw)>0.02 && ovl(y,y+h,fy,fy+fh)>0.02 && !inNest(fx,fx+fw,fy,fy+fh) && k!=='stairCab')
        e.push(`Cầu thang ${s.id} chồng nội thất ${k} @${fx},${fy}`);
    for(const z of swings)
      if(ovl(x,x+w,z.x0,z.x1)>0.02 && ovl(y,y+h,z.y0,z.y1)>0.02 && !inNest(z.x0,z.x1,z.y0,z.y1))
        e.push(`${z.id} quét vào cầu thang ${s.id}`);
  }
