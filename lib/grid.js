...
import { LOT, TANK, GARDEN } from './lot.js';
...
export function toGrid(plan){
  ...
  /* Diềm mái hiên chọn bản theo tường nó bám — neo như lớp chống nóng. */
  const overhangFascias = plan.overhangFascias?.map(([ax, pos]) =>
    ({ ax, pos: ax === 'h' ? Y(pos, 'diềm mái hiên') : X(pos, 'diềm mái hiên') }));
  ...
  /* Bồn nước áp sát một cạnh của sân / phòng nó đứng bên trên: neo vào **cạnh gần nhất của phòng ấy**, giữ
     nguyên khoảng chừa; kích thước là của cái bồn (TANK), không co giãn theo lưới.

     Neo vào đường lưới gần nhất là sai ở đây: bồn rộng gần 1 m nên mép kia của nó luôn rơi cạnh một đường
     vu vơ — bồn áp tường `x = 5` mà mép trái lại gần đường 3.66 (đầu trong bức lửng) hơn, kéo tường `x = 5`
     là bồn đứng yên, tách khỏi bức tường nó dựa vào. */
  const hugAnchor = (lines, ax, a, b, ca, cb) => {
    const r = plan.rooms.find(rm => {
      const [p0, p1, q0, q1] = ax === 'x'
        ? [rm[2], rm[2] + rm[4], rm[3], rm[3] + rm[5]] : [rm[3], rm[3] + rm[5], rm[2], rm[2] + rm[4]];
      return a >= p0 - EPS && b <= p1 + EPS && ca >= q0 - EPS && cb <= q1 + EPS;
    });
    if (!r) return nearAnchor(lines, a);
    const edges = ax === 'x' ? [r[2], r[2] + r[4]] : [r[3], r[3] + r[5]];
    const gap = v => Math.min(Math.abs(a - v), Math.abs(b - v));
    const pick = gap(edges[1]) < gap(edges[0]) ? edges[1] : edges[0];
    const base = findLine(lines, pick);
    return base < 0 ? nearAnchor(lines, a) : { base, off: a - pick };
  };
  const tanks = plan.tanks?.map(([id, x, y, axis]) => {
    const [w, h] = axis === 'x' ? [TANK.len, TANK.dia] : [TANK.dia, TANK.len];
    return { id, axis, x: hugAnchor(xs, 'x', x, x + w, y, y + h), y: hugAnchor(ys, 'y', y, y + h, x, x + w) };
  });
  ...
  /* Chậu cây neo như bồn nước — vào **cạnh gần nhất của sân chứa nó**, giữ khoảng chừa; cỡ chậu là của GARDEN.
     Chậu hai bên bậc cửa chính áp mặt tường nhà `y = 12`: dịch tường ấy thì bậc đi theo, chậu cũng phải đi theo.
     Bồn hoa xây áp tường: tuyến bám tường, **hai đầu** neo mặt đường gần nhất — bồn xây tại chỗ, không phải đồ mua
     sẵn như giàn phơi, nên đầu nào dựa vào mốc nào (sau trụ cổng, mép mái hiên) thì đi theo mốc ấy. */
  const planters = plan.planters?.map(([id, x, y, ...rest]) => {
    const r = GARDEN.pot.dia / 2;
    return { id, x: hugAnchor(xs, 'x', x - r, x + r, y - r, y + r), y: hugAnchor(ys, 'y', y - r, y + r, x - r, x + r), rest };
  });
  /* Cây bóng mát neo như chậu — theo ô gốc, vào cạnh gần nhất của sân chứa nó. */
  const trees = plan.trees?.map(([id, x, y, ...rest]) => {
    const r = GARDEN.tree.pit / 2;
    return { id, x: hugAnchor(xs, 'x', x - r, x + r, y - r, y + r), y: hugAnchor(ys, 'y', y - r, y + r, x - r, x + r), rest };
  });
  const flowerBeds = plan.flowerBeds?.map(([id, ax, pos, a, b, ...rest]) => {
    const [across, along] = ax === 'h' ? [ys, xs] : [xs, ys];
    return { id, ax, pos: nearAnchor(across, pos), a: nearAnchor(along, a), b: nearAnchor(along, b), rest };
  });

  /* Đèn trần neo vào gốc phòng chứa tâm đèn như nội thất — phòng kín trước, vì WC khách nằm lọt trong sân phơi:
     neo vào gốc sân phơi thì dịch tường WC khách là đèn WC đứng lại. Đèn tường bám bức tường của nó: vị trí trục
     bám đường lưới (tường tum không nằm trên đường nào thì mặt đường gần nhất), vị trí dọc tường neo vào gốc phòng
     phía đèn quay vào. */
  const roomOfPoint = (x, y) => plan.rooms.filter(r => holds(r, x, y, 0, 0))
    .sort((p, q) => (q[6] !== 'yard' && q[0] !== 'R3') - (p[6] !== 'yard' && p[0] !== 'R3') || p[4] * p[5] - q[4] * q[5])[0];
  const pointAnchor = (x, y) => {
    const r = roomOfPoint(x, y);
    return r ? { x: { base: X(r[2], 'gốc phòng x'), off: x - r[2] }, y: { base: Y(r[3], 'gốc phòng y'), off: y - r[3] } }
             : { x: anchor(xs, x), y: anchor(ys, y) };
  };
  const lights = plan.lights?.map(([id, type, x, y, ...rest]) => ({ id, type, ...pointAnchor(x, y), rest }));
  const wallAnchor = (ax, pos, u, dir) => {
    const across = ax === 'h' ? ys : xs, i = findLine(across, pos);
    const side = pos + dir * 0.12;
    const p = pointAnchor(ax === 'h' ? u : side, ax === 'h' ? side : u);
    return { pos: i >= 0 ? { base: i, off: 0 } : nearAnchor(across, pos), at: ax === 'h' ? p.x : p.y };
  };
  const wallLights = plan.wallLights?.map(([id, type, ax, pos, u, dir, ...rest]) =>
    ({ id, type, ax, dir, ...wallAnchor(ax, pos, u, dir), rest }));
  /* Bảng công tắc neo hệt đèn tường. */
  const switches = plan.switches?.map(([id, ax, pos, u, dir, ...rest]) => ({ id, ax, dir, ...wallAnchor(ax, pos, u, dir), rest }));

  return { meta: plan, xs, ys, rooms, walls, doors, windows, gates, skylights, furn, strips, dims,
           overhangs, fullHeightWalls, solidFences, roofs, posts, roofInsulation, overhangFascias, stairs, tum, roofRailings, tanks, racks,
           planters, flowerBeds, trees, lights, wallLights, switches };
}
...
function areasOf(rooms, underStairs = []){
  const A = r => r[4] * r[5];
  const nested = new Set(underStairs.map(u => u.room));   // phòng gầm thang lồng trong buồng thang, không cộng riêng
  const kin = rooms.filter(r => r[6] !== 'yard' && r[0] !== 'R3' && !nested.has(r[0])).reduce((s, r) => s + A(r), 0);
...
export function fromGrid(g){
  ...
  const overhangFascias = g.overhangFascias && g.overhangFascias.map(f => [f.ax, f.ax === 'h' ? ys[f.pos] : xs[f.pos]]);
  ...
  const planters = g.planters && g.planters.map(p => {
    const r = GARDEN.pot.dia / 2;
    return [p.id, unanchor(xs, p.x) + r, unanchor(ys, p.y) + r, ...p.rest];
  });
  const trees = g.trees && g.trees.map(t => {
    const r = GARDEN.tree.pit / 2;
    return [t.id, unanchor(xs, t.x) + r, unanchor(ys, t.y) + r, ...t.rest];
  });
  const flowerBeds = g.flowerBeds && g.flowerBeds.map(b => {
    const [across, along] = b.ax === 'h' ? [ys, xs] : [xs, ys];
    return [b.id, b.ax, unanchor(across, b.pos), unanchor(along, b.a), unanchor(along, b.b), ...b.rest];
  });

  const lights = g.lights && g.lights.map(l => [l.id, l.type, unanchor(xs, l.x), unanchor(ys, l.y), ...l.rest]);
  const wallLights = g.wallLights && g.wallLights.map(l => {
    const [across, along] = l.ax === 'h' ? [ys, xs] : [xs, ys];
    return [l.id, l.type, l.ax, unanchor(across, l.pos), unanchor(along, l.at), l.dir, ...l.rest];
  });
  const switches = g.switches && g.switches.map(s => {
    const [across, along] = s.ax === 'h' ? [ys, xs] : [xs, ys];
    return [s.id, s.ax, unanchor(across, s.pos), unanchor(along, s.at), s.dir, ...s.rest];
  });

  return { ...g.meta, rooms, walls, doors, windows, gates, skylights, furn, strips, dims,
           ...(lights ? { lights } : {}), ...(wallLights ? { wallLights } : {}), ...(switches ? { switches } : {}),
           ...(overhangs ? { overhangs } : {}), ...(fullHeightWalls ? { fullHeightWalls } : {}), ...(solidFences ? { solidFences } : {}),
           ...(roofs ? { roofs } : {}), ...(posts ? { posts } : {}),
           ...(roofInsulation ? { roofInsulation } : {}), ...(overhangFascias ? { overhangFascias } : {}),
           ...(stairs ? { stairs } : {}), ...(tum ? { tum } : {}),
           ...(roofRailings ? { roofRailings } : {}), ...(tanks ? { tanks } : {}), ...(racks ? { racks } : {}),
           ...(planters ? { planters } : {}), ...(flowerBeds ? { flowerBeds } : {}), ...(trees ? { trees } : {}),
           areas: areasOf(rooms, g.meta.underStairs) };
}
