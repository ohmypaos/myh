...
export function toGrid(plan){
  ...
  const solidFences = plan.solidFences?.map(([ax, pos, a, b]) => ax === 'h'
    ? { ax, pos: Y(pos, 'rào kín'), a: X(a, 'rào kín a'), b: X(b, 'rào kín b') }
    : { ax, pos: X(pos, 'rào kín'), a: Y(a, 'rào kín a'), b: Y(b, 'rào kín b') });
...
export function fromGrid(g){
  ...
  const solidFences = g.solidFences && g.solidFences.map(w => w.ax === 'h'
