...
import { stepsOf, overhangsOf, lightRoofs, roofPanels, roofOver, floorOf, gutters, downpipes, postsOf, beamsOf, purlinsOf, dropCeilingsOf, roofInsulationOf } from './envelope.js';
import { ROOF, ROOF_INSULATION, RAILING, POST, BEAM, PURLIN, heightsOf } from './lot.js';
...
export function specMarkdown(v, PLANS){
  ...
  if(ins){
    ...
    s+=`\n### Lớp chống nóng mái\n\nKiểu **mái ngược**: chống thấm nằm dưới tấm cách nhiệt, được che nắng và đỡ co giãn nhiệt. `
     ...
     +`tới mặt ngoài tường; chừa giếng trời — quanh mỗi lỗ xây gờ chắn nước cao hơn mặt gạch. `
     +`Diện tích lát **${n(area)} m²**, mặt gạch cốt **${n(top+ins.thickness)}** (bản mái ${n(top)}, chưa tính vữa tạo dốc).\n\n`;
    ...
    s+=`| Vải địa kỹ thuật | — | ngăn vữa chảy vào khe tấm |\n`;
    s+=`| Vữa cán lưới thép | ${n(I.screed)} | khe co giãn cách nhau ≤ ${n(I.joint,1)} m, chèn keo đàn hồi |\n`;
    s+=`| Gạch lát | ${n(I.tile)} | |\n`;
  }
