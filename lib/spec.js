...
export function specMarkdown(v, PLANS){
  ...
  if(steps.length){
    s+=`\n### Bậc tam cấp\n\nVị trí suy từ cửa, đặt phía sân. Toạ độ tính từ tim tường. `
     +`Số bậc không tính nấc trên cùng — nấc đó chính là sàn nhà.\n\n`;
    s+=`| Cửa | Xuống | x₁ | y₁ | x₂ | y₂ | Số bậc | Cao bậc | Mặt bậc |\n|---|---|---:|---:|---:|---:|---:|---:|---:|\n`;
    for(const b of steps){
      const r=b.rect, room=v.rooms.find(x=>x[0]===b.room);
      s+=`| ${b.id} | ${room[1]} | ${n(r.x)} | ${n(r.y)} | ${n(r.x+r.w)} | ${n(r.y+r.h)} | ${b.count} | ${n(b.rise)} | ${n(b.tread)} |\n`;
    }
  }
  if(overs.length){
