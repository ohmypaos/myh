...
import { stepsOf, overhangsOf, lightRoofs, roofPanels, roofOver, floorOf, gutters, downpipes, postsOf, beamsOf, purlinsOf, dropCeilingsOf, roofInsulationOf,
         ...
         plantersOf, flowerBedsOf, treesOf, fasciasOf, isUnderStair } from './envelope.js';
...
export function specMarkdown(v, PLANS){
  ...
  const enclosedRooms=v.rooms.filter(r=>r[6]!=='yard'&&r[0]!=='R3'&&!isUnderStair(v,r));   // phòng gầm thang lồng trong buồng thang
  ...
  const sumL=v.rooms.filter(r=>r[0][0]==='L'&&!isUnderStair(v,r)).reduce((a,r)=>a+r[4]*r[5],0);
  ...
  if(steps.length){
    ...
    for(const b of steps){
      ...
      s+=`| ${b.id} | ${room[1]} | ${n(r.x)} | ${n(r.y)} | ${n(r.x+r.w)} | ${n(r.y+r.h)} | ${Array.isArray(b.margin) ? b.margin.map(m=>n(m)).join(' / ') : n(b.margin)} | ${b.count} | ${n(b.rise)} | ${n(b.tread)} |\n`;
    }
  }
  if(overs.length){
