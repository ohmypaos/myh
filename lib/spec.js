...
import { stepsOf, overhangsOf, lightRoofs, roofPanels, roofOver, floorOf, gutters, downpipes, postsOf, dropCeilingsOf } from './envelope.js';
import { ROOF, RAILING, POST, heightsOf } from './lot.js';
  ...
  if(roofs.length){
    ...
    if(cs.length){
      s+=`\n**Cột đỡ mái nhẹ** — ${POST.mat}, đứng ở chỗ mép mái không tựa lên tường cao tới mái. Vị trí khai `
       +`theo tâm cột; chiều cao suy ra: từ cốt sân tới mặt dưới mái ngay trên cột, hoặc đáy máng nếu máng chạy qua `
       +`đầu cột. Nhịp giữa hai chỗ đỡ dọc một mép mái không quá ${n(POST.maxSpan,1)} m.\n\n`;
      s+=`| Mã | x | y | Đỉnh cột | Đỡ mái | Gác máng |\n|---|---:|---:|---:|---|---|\n`;
      for(const c of cs)
        s+=`| ${c.id} | ${n(c.x)} | ${n(c.y)} | ${n(c.top)} | ${c.roofs.join(', ')} | ${c.gutter?'có':'—'} |\n`;
    }
  }
