...
export function specMarkdown(v, PLANS){
  ...
  if((v.solidFences||[]).length)
    s+=`**Đoạn rào xây kín** — không lan can, đỉnh **${n(H.fencePrivate)} m** so với sân, để che tầm nhìn: `
     +v.solidFences.map(([ax,pos,a,b])=>`trục ${axisName(ax)} ${n(pos,1)} (${n(a,1)}–${n(b,1)})`).join('; ')
