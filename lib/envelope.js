...
import { LOT, STEP, ROOF, ROOF_INSULATION, POST, BEAM, PURLIN, STAIR, TUM, ROOF_RAILING, heightsOf } from './lot.js';
...
export function roofWalkTop(v){
  ...
  return L.houseTop + (v.roofInsulation ? I.xps + I.screed + I.tile : 0);
}
...
export function roofRailingsOf(v){
  ...
  return (v.roofRailings || []).map(([ax, pos, a, b]) => {
    ...
    return { ax, pos, a, b, y0, y1, error: off == null ? null : `Lan can mái trục ${ax} ${pos} (${a}–${b}) ra ngoài mái bê tông quanh ${off.toFixed(2)}` };
  });
}
