...
import { LOT, STEP, ROOF, POST, heightsOf } from './lot.js';
...
/* ═══════════ CỘT ĐỠ MÁI NHẸ ═══════════
   ...
   cao thì suy: đỉnh cột chạm chỗ thấp nhất của mặt dưới các tấm mái phủ lên tiết diện cột — hoặc đáy máng
   nếu máng chạy qua đầu cột (cột góc mép trước mái sân chính), máng gác lên đầu cột. */
export function postsOf(v){
  const panels = roofPanels(v), gs = gutters(v), S = POST.size;
  return (v.posts || []).map(([id, x, y]) => {
    ...
    return { id, x, y, size: S, rect, top: Math.min(...unders, ...drops),
             roofs: [...new Set(above.map(p => p.id))], gutter: drops.length > 0 };
  });
}
