...
import { LOT, STEP, ROOF, ROOF_INSULATION, POST, BEAM, PURLIN, RAILING, STAIR, TUM, TANK, RACK, ROOF_RAILING, GATE_ROOF, GATE_DOOR,
         GARDEN, FASCIA, LIGHT, LIGHTING, SWITCH, FURNITURE, heightsOf } from './lot.js';
...
/* Mái cổng bám vào đúng khoảng mở cổng, nên kéo lưới làm cổng dịch / đổi rộng thì hai
   trụ và mái tự theo. `run` là nửa bề sâu: một nửa chìa qua ranh ra đường, một nửa vào sân. */
export function gateRoofsOf(v){
  return (v.gateRoofs || []).map((spec, i) => {
    const g = v.gates?.[spec.gate];
    const id = spec.id || `GC${i + 1}`;
    if (!g) return { id, error:`Mái cổng ${id}: không có cổng số ${spec.gate}` };
    const [ax, pos, a, b] = g;
    const R = { ...GATE_ROOF, ...spec, id, ax, pos, a, b, ridge:GATE_ROOF.eave + GATE_ROOF.rise };
    R.ridge = R.eave + R.rise;
    if (!['h', 'v'].includes(ax)) R.error = `Mái cổng ${id}: trục cổng '${ax}' phải là h / v`;
    else if (!(b > a && R.pillar > 0 && R.run > 0 && R.eave > 0 && R.rise > 0 && R.tile > 0))
      R.error = `Mái cổng ${id}: kích thước trụ hoặc mái không hợp lệ`;
    if (R.error) return R;
    /* [a,b] là khoảng thông xe, nên hai trụ nằm hẳn ở phía ngoài hai mép này — không đặt
       tim trụ vào mép rồi vô tình thu hẹp cổng. */
    const end0 = a - R.pillar, end1 = b + R.pillar;
    R.rect = ax === 'h'
      ? { x:end0 - R.overhang, z:pos - R.run, w:end1 - end0 + 2 * R.overhang, d:2 * R.run }
      : { x:pos - R.run, z:end0 - R.overhang, w:2 * R.run, d:end1 - end0 + 2 * R.overhang };
    const half = R.pillar / 2;
    R.pillars = ax === 'h'
      ? [{ x:a - R.pillar, z:pos - half }, { x:b, z:pos - half }]
      : [{ x:pos - half, z:a - R.pillar }, { x:pos - half, z:b }];
    return R;
  });
}

/* Cánh cổng bám vào khoảng thông thực [a,b], còn hai trụ ở phía ngoài khoảng này. */
export function gateDoorsOf(v){
  return (v.gateDoors || []).map((spec, i) => {
    const g = v.gates?.[spec.gate];
    const id = spec.id || `CG${i + 1}`;
    if (!g) return { id, error:`Cánh cổng ${id}: không có cổng số ${spec.gate}` };
    const [ax, pos, a, b] = g;
    const R = { ...GATE_DOOR, ...spec, id, ax, pos, a, b, mid:(a + b) / 2 };
    if (!['h', 'v'].includes(ax)) R.error = `Cánh cổng ${id}: trục cổng '${ax}' phải là h / v`;
    else if (!(b > a && R.height > 0 && R.frame > 0 && R.slat > 0 && R.slatPitch > R.slat))
      R.error = `Cánh cổng ${id}: kích thước khung hoặc nan không hợp lệ`;
    return R;
  });
}
...
/* ═══════════ BẬC TAM CẤP ═══════════
   `v.steps` là { mã cửa: { margin, tread } } — `margin` là phần bậc rộng hơn cửa mỗi bên (một số, hoặc
   `[phía a, phía b]` khi một bên áp tường — bậc D2 chạy sát mặt tường bếp, không chừa góc thừa),
...
export function stepsOf(v){
  ...
  return Object.entries(v.steps || {}).flatMap(([id, { margin = 0, tread = STEP.tread } = {}]) => {
    ...
    const [ma, mb] = Array.isArray(margin) ? margin : [margin, margin];
    const a = da - ma, b = db + mb, depth = count * tread;
...
/* `v.stairs`: { id, name, room, at:[x0,y0,x1,y1], width, tread, risers:[vế 1, vế 2], flip? } — cầu thang chữ U từ sàn
   phòng `room` lên mặt mái. `at` là **lọt lòng buồng thang** (mặt tường, mép lửng), cũng là lỗ khoét bản mái.
   Chiếu nghỉ ở đầu x nhỏ, vế 1 leo về phía x nhỏ, vế 2 leo về phía x lớn và ra tới đúng mép x lớn của lỗ — bước lên
   bản mái. Mặc định vế 2 dọc mép y nhỏ, vế 1 dọc mép y lớn (bản hiện hành); `flip: true` đổi chỗ hai dải. Vế nào cũng
   rộng `width`, khe giữa hai vế là phần còn lại. Mỗi vế `risers` nấc; nấc cuối vế 1 lên chiếu nghỉ, nấc cuối vế 2 lên
   mặt mái, nên số mặt bậc là `risers − 1`. Cao bậc suy ra, không khai.

   Trả kèm vị trí hai dải (`band1`, `band2`), mép trong giáp khe (`inner1`, `inner2`), mép ngoài (`outer1`, `outer2`),
   `side2` (−1 nếu vế 2 ở phía y nhỏ) — để lan can, nét vẽ và phép kiểm không phải đoán chiều. `outerRail`: mép ngoài
   vế 2 có cần lan can không — suy ra, cần khi phía ấy không có tường cao (bức lửng `low` hoặc để hở). */
export function stairsOf(v){
  ...
  return (v.stairs || []).map(s => {
    const { id, name, room: roomId, at: [x0, y0, x1, y1], width, tread, risers: [r1, r2], flip = false } = s;
    ...
    const side2 = flip ? 1 : -1;
    const band2 = flip ? [y1 - width, y1] : [y0, y0 + width], band1 = flip ? [y0, y0 + width] : [y1 - width, y1];
    const inner2 = flip ? band2[0] : band2[1], inner1 = flip ? band1[1] : band1[0];
    const outer2 = flip ? y1 : y0, outer1 = flip ? y0 : y1;
    /* Tường có mặt trùng mép ngoài vế 2: không có, hoặc chỉ là bức lửng, thì mép ấy hở — cần lan can. */
    const edgeWall = v.walls.find(([wax, wpos, wa, wb, wt]) => wax === 'h'
      && Math.abs(wpos - side2 * wt / 2 - outer2) < 0.02 && overlap(wa, wb, x0, x1) > EPS);
    const outerRail = !edgeWall || edgeWall[5] === 'low';

    const landingTop = base + r1 * rise;
    const treads = [];
    for (let j = 0; j < r1 - 1; j++)
      treads.push({ flight: 1, x: start1 - (j + 1) * tread, y: band1[0], w: tread, h: width, top: base + (j + 1) * rise });
    for (let j = 0; j < r2 - 1; j++)
      treads.push({ flight: 2, x: x0 + width + j * tread, y: band2[0], w: tread, h: width, top: landingTop + (j + 1) * rise });
    return { id, name, room: roomId, base, top, rise, tread, width, risers: [r1, r2], start1, landingTop, treads,
             flip, side2, band1, band2, inner1, inner2, outer1, outer2, outerRail,
...
/* `v.underStairs`: [{ room, stair }] — phòng kín **lồng dưới gầm** một cầu thang (bản nháp stair-back: WC chung dưới vế 2).
   Phòng lồng nằm trọn trong phòng buồng thang nên không cộng diện tích riêng, không đổ sàn hay bản mái riêng; trần của nó
   là mặt dưới bậc — đèn trần bám vào đó, nội thất và cánh cửa trong nó không coi là chồng cầu thang. Vách của nó đứng trong
   lỗ thang thì dừng ngay dưới bậc (massing.js). Khoảng đầu thật dưới gầm do check-3d soi trên khối đã dựng. */
export const isUnderStair = (v, r) => (v.underStairs || []).some(u => u.room === r[0]);
export function underStairsOf(v){
  return (v.underStairs || []).map(({ room: id, stair }) => {
    const r = v.rooms.find(x => x[0] === id), s = stairsOf(v).find(x => x.id === stair);
    if (!r) return { room: id, stair, error: `Phòng gầm thang ${id}: không có phòng mã này` };
    if (!s || s.error) return { room: id, stair, error: `Phòng gầm thang ${id}: không có cầu thang ${stair}` };
    if (!isEnclosed(r)) return { room: id, stair, error: `Phòng gầm thang ${id}: ${r[1]} phải là phòng kín` };
    const host = v.rooms.find(x => x[0] === s.room), [, , x, y, w, h] = r;
    if (!host || !(x >= host[2] - EPS && y >= host[3] - EPS && x + w <= host[2] + host[4] + EPS && y + h <= host[3] + host[5] + EPS))
      return { room: id, stair, error: `Phòng gầm thang ${id} không nằm trọn trong buồng thang ${s.room}` };
    return { room: id, stair, host: s.room, rect: { x, y, w, h } };
  });
}

/* Tủ module dưới gầm thang (`furn` loại `stairCab`): hộp chữ nhật đặt dưới một vế hay chiếu nghỉ, không vách không cửa.
   **Cao suy ra**, không khai: tới sát mặt dưới bậc thấp nhất phủ lên nó (cùng mặt dưới như massing.js buildStairs), chừa
   FURNITURE.stairCab.gap. Xếp nhiều hộp nối nhau dưới một vế là ra dãy tủ bậc cấp. */
export function stairCabinetOf(v, [, x, y, w, h]){
  const H = heightsOf(v), C = FURNITURE.stairCab;
  const hits = (px, pw, pz, pd) => overlap(x, x + w, px, px + pw) > EPS && overlap(y, y + h, pz, pz + pd) > EPS;
  const under = stairsOf(v).filter(s => !s.error).flatMap(s => [
    ...s.treads.filter(t => hits(t.x, t.w, t.y, t.h)).map(t => Math.max(s.base, t.top - s.rise - STAIR.flight)),
    ...(hits(s.landing.x, s.landing.w, s.landing.y, s.landing.h) ? [s.landing.top - s.rise - STAIR.flight] : []),
  ]);
  if (!under.length) return { height: 0, error: `Tủ gầm thang @${x},${y} không nằm dưới cầu thang nào` };
  const room = roomAtPoint(v, x + w / 2, y + h / 2), floor = room ? floorOf(v, room, H.floor) : 0;
  const height = Math.min(...under) - floor - C.gap;
  if (height < C.min - EPS)
    return { height, error: `Tủ gầm thang @${x},${y} chỉ cao ${height.toFixed(2)} m dưới bậc, thấp hơn ${C.min} m` };
  return { height };
}
...
export function tumOf(v){
  ...
  const errors = [];
  /* Tum phải **ôm sát** lỗ thang, không chỉ trùm kín nó: mặt trong mỗi vách cách mép lỗ không quá một bề
     dày vách. Cho phép hụt chút vì vách tum mỏng hơn bức tường nó đứng lên (vách 100 trên tường bao 220 thì
     mặt trong lùi 0.06) — nhưng hở hơn thế là thừa một dải bản mái nằm trong tum, mưa không vào mà cũng
     chẳng để làm gì, và là dấu hiệu tum khai lệch khỏi lỗ thang. */
  const hw0 = TUM.wall / 2;
  for (const s of stairsOf(v).filter(s => !s.error)) {
    const hl = s.hole;
    if (hl.x < x0 || hl.y < y0 || hl.x + hl.w > x1 || hl.y + hl.h > y1) { errors.push(`Tum không trùm kín lỗ thang ${s.id}`); continue; }
    for (const [name, gap] of [['tây', hl.x - (x0 + hw0)], ['đông', (x1 - hw0) - (hl.x + hl.w)],
                               ['bắc', hl.y - (y0 + hw0)], ['nam', (y1 - hw0) - (hl.y + hl.h)]])
      if (gap > TUM.wall + EPS)
        errors.push(`Vách ${name} của tum cách mép lỗ thang ${s.id} ${gap.toFixed(2)} m — thừa một dải bản mái trong tum`);
  }
  ...
  for (const s of stairsOf(v).filter(s => !s.error)) {
    ...
    else if (!doors.some(d => d.ax === 'v' && Math.abs(d.pos - edge) < EPS && d.a >= s.band2[0] - EPS && d.b <= s.band2[1] + EPS))
...
export function roofSlabs(v){
  return [
    ...v.rooms.filter(r => isEnclosed(r) && !roofOver(v, r) && !isUnderStair(v, r)).map(([, , x, y, w, h]) => ({ x, y, w, h })),
...
export function tanksOf(v){
  ...
  return (v.tanks || []).map(([id, x, y, axis]) => {
    ...
    const legTop = standTop - T.cradle;
    const legAt = (lx, ly) => ({ x: lx, y: ly, w: T.leg, h: T.leg, y0: base + T.padThick, y1: legTop });
    ...
    /* Hai bộ kiềng: mỗi thanh bắc ngang hai chân cùng một đầu, **vuông góc trục bồn**, mặt trên đúng cốt đáy
       trụ — bồn tì lên đây theo đường tim đáy. Không có nó thì chân đứng ở bốn góc chống hụt vào mặt cong. */
    const ends = axis === 'x' ? [x + m, x + w - m - T.leg] : [y + m, y + h - m - T.leg];   // dọc trục
    const span = axis === 'x' ? [y + m, y + h - m] : [x + m, x + w - m];                   // ngang trục
    const cradles = ends.map(u => axis === 'x'
      ? { x: u, y: span[0], w: T.leg, h: span[1] - span[0], y0: legTop, y1: standTop }
      : { x: span[0], y: u, w: span[1] - span[0], h: T.leg, y0: legTop, y1: standTop });
    return { id, axis, x, y, w, h, base, standTop, legTop, top, volume: T.volume,
             legs: cs.map(([a, b]) => legAt(a, b)), pads: cs.map(([a, b]) => padAt(a, b)), cradles, errors };
  });
}

/* `v.solidFences`: [[trục, vị trí, từ, đến, đỉnh?], …] — đoạn tường rào **xây kín hết, không lan can**,
   để che tầm nhìn. Đỉnh khai riêng từng đoạn vì hai cạnh lộ khác nhau thì cần khác cao; bỏ trống thì lấy
   `heights.fencePrivate`. Chiều cao đo từ cốt sân — chỗ nào sàn cao hơn sân (ban công) thì rào nhô lên ít
   hơn đúng bằng chênh cốt, xem `3d.md` mục 2a. */
export function solidFencesOf(v){
  const H = heightsOf(v);
  return (v.solidFences || []).map(([ax, pos, a, b, top]) => ({ ax, pos, a, b, top: top ?? H.fencePrivate }));
}
...
/* ═══════════ CÂY XANH ═══════════
   Phối cảnh ngoài sân (GARDEN trong lib/lot.js). Chậu cây và bồn hoa phải nằm gọn trong **phần lọt lòng** của một
   cái sân — không lấn vào bề dày tường — và không đè lên phòng kín nào lọt trong sân ấy (WC khách nằm trong sân
   phơi). Chân đứng trên cốt sân của sân chứa nó. Chồng bậc, nội thất, cột, trụ cổng, vùng quét cánh cửa thì
   validate() soi, như giàn phơi. */
function yardHolding(v, { x, y, w, h }){
  const yard = v.rooms.find(r => {
    if (r[6] !== 'yard') return false;
    const c = clearRect(v, r);
    return x >= c.x - EPS && y >= c.y - EPS && x + w <= c.x + c.w + EPS && y + h <= c.y + c.h + EPS;
  }) || null;
  const inside = yard && v.rooms.find(r => isEnclosed(r)
    && overlap(x, x + w, r[2], r[2] + r[4]) > EPS && overlap(y, y + h, r[3], r[3] + r[5]) > EPS);
  return { yard, inside };
}

/* `v.planters`: [[mã, x, y, mô tả], …] — chậu cây đặt rời, (x, y) là **tâm chậu**. `rect` là thân chậu, `reach` là
   hình chiếu cả tán lẫn chậu — tán rộng hơn miệng chậu nên soi chạm tường, chạm bậc theo `reach`. */
export function plantersOf(v){
  const P = GARDEN.pot, H = heightsOf(v), seen = new Set();
  return (v.planters || []).map(([id, x, y, desc]) => {
    const errors = [];
    if (seen.has(id)) errors.push(`Chậu cây ${id}: trùng mã`); else seen.add(id);
    const square = d => ({ x: x - d / 2, y: y - d / 2, w: d, h: d });
    const rect = square(P.dia), reach = square(Math.max(P.dia, P.crown));
    const { yard, inside } = yardHolding(v, reach);
    if (!yard) errors.push(`Chậu cây ${id} ở ${x.toFixed(2)}, ${y.toFixed(2)} không nằm gọn trong lọt lòng một cái sân`);
    else if (inside) errors.push(`Chậu cây ${id} đè lên ${inside[1]}`);
    const base = yard ? floorOf(v, yard, H.floor) : 0, top = base + P.height;
    const crown = { ...square(P.crown), y0: top - P.sink, y1: top + P.plant };
    return { id, x, y, rect, reach, base, top, crown, room: yard?.[0], desc, errors };
  });
}

/* `v.trees`: [[mã, x, y, mô tả], …] — cây bóng mát trồng xuống đất, (x, y) là **tâm gốc**. Ô gốc vuông bó vỉa nằm gọn
   trong lọt lòng một sân (`reach` là ô gốc — thứ chiếm chỗ dưới chân); tán ở trên đầu nên không soi chồng bậc hay đồ
   đạc, nhưng **không được vươn ra ngoài lô**. Tán đâm vào mái, tường thì check-3d phép 10 bắt trên khối đã dựng. */
export function treesOf(v){
  const T = GARDEN.tree, H = heightsOf(v), seen = new Set();
  return (v.trees || []).map(([id, x, y, desc]) => {
    const errors = [];
    if (seen.has(id)) errors.push(`Cây ${id}: trùng mã`); else seen.add(id);
    const square = d => ({ x: x - d / 2, y: y - d / 2, w: d, h: d });
    const pit = square(T.pit), r = T.crown / 2;
    const { yard, inside } = yardHolding(v, pit);
    if (!yard) errors.push(`Cây ${id} ở ${x.toFixed(2)}, ${y.toFixed(2)}: ô gốc không nằm gọn trong lọt lòng một cái sân`);
    else if (inside) errors.push(`Cây ${id}: ô gốc đè lên ${inside[1]}`);
    if (x - r < -EPS || y - r < -EPS || x + r > LOT.w + EPS || y + r > LOT.d + EPS)
      errors.push(`Cây ${id}: tán Ø${T.crown} vươn ra ngoài lô — sang đất bên cạnh hoặc ra ngõ`);
    const base = yard ? floorOf(v, yard, H.floor) : 0, curbTop = base + T.pitHeight, soilTop = curbTop - T.soil;
    const c = T.pitCurb, inner = T.pit - 2 * c;
    const at = (x0, y0, w, h) => ({ x: pit.x + x0, y: pit.y + y0, w, h, y0: base, y1: curbTop });
    /* Hai bó vỉa x chạy suốt cạnh, hai bó vỉa y lọt giữa. */
    const curbs = [at(0, 0, T.pit, c), at(0, T.pit - c, T.pit, c), at(0, c, c, inner), at(T.pit - c, c, c, inner)];
    const soil = { ...square(inner), y0: base, y1: soilTop };
    const trunk = { ...square(T.trunk), y0: soilTop - T.soil, y1: base + T.clear + 0.40 };
    const crown = { ...square(T.crown), y0: base + T.clear, y1: base + T.clear + T.crownHeight };
    return { id, x, y, pit, reach: pit, base, curbTop, soilTop, curbs, soil, trunk, crown, top: crown.y1,
             room: yard?.[0], desc, errors };
  });
}

/* `v.flowerBeds`: [[mã, trục, vị trí, từ, đến, mô tả], …] — bồn hoa xây **áp tường**, cùng dạng khai với tuyến tường.
   Phía bồn suy ra: phía bức tường có sân. Lưng bồn là mặt tường, ba phía bó vỉa; đất trồng thấp hơn mép bó vỉa, khóm
   hoa chia đều dọc bồn. Bồn không được chắn cửa hay cổng trên chính bức tường nó áp. */
export function flowerBedsOf(v){
  const B = GARDEN.bed, H = heightsOf(v), seen = new Set();
  return (v.flowerBeds || []).map(([id, ax, pos, a, b, desc]) => {
    const errors = [];
    const out = { id, ax, pos, a, b, desc, errors, curbs: [], soil: null, clumps: [], blooms: [] };
    if (seen.has(id)) errors.push(`Bồn hoa ${id}: trùng mã`); else seen.add(id);
    if (!['h', 'v'].includes(ax)) { errors.push(`Bồn hoa ${id}: trục '${ax}' phải là 'h' hoặc 'v'`); return out; }
    if (b - a < 2 * B.curb + B.clump) { errors.push(`Bồn hoa ${id}: dài ${(b - a).toFixed(2)} m, không đủ xây`); return out; }
    if (!v.walls.some(([wax, wpos, wa, wb]) => wax === ax && Math.abs(wpos - pos) < EPS && wa <= a + EPS && wb >= b - EPS))
      { errors.push(`Bồn hoa ${id} trục ${ax} ${pos} (${a}–${b}) không áp trọn một bức tường`); return out; }
    const sides = new Set(roomsAlong(v.rooms, ax, pos, a, b).filter(r => r[6] === 'yard').map(r => sideOf(r, ax, pos)));
    if (sides.size !== 1)
      { errors.push(`Bồn hoa ${id}: ${sides.size ? 'hai phía tường đều là sân, không biết xây phía nào' : 'tường không áp vào sân nào'}`); return out; }
    const dir = [...sides][0], face = pos + dir * wallThickness(v, ax, pos, a, b) / 2;
    const rect = band(ax, face, a, b, dir, B.width);
    const { yard, inside } = yardHolding(v, rect);
    if (!yard) errors.push(`Bồn hoa ${id} không nằm gọn trong lọt lòng một cái sân`);
    else if (inside) errors.push(`Bồn hoa ${id} đè lên ${inside[1]}`);
    for (const [did, dax, dpos, da, db] of v.doors)
      if (dax === ax && Math.abs(dpos - pos) < EPS && overlap(a, b, da, db) > EPS) errors.push(`Bồn hoa ${id} chắn cửa ${did}`);
    for (const [gax, gpos, ga, gb, name] of v.gates)
      if (gax === ax && Math.abs(gpos - pos) < EPS && overlap(a, b, ga, gb) > EPS) errors.push(`Bồn hoa ${id} chắn ${name}`);

    const base = yard ? floorOf(v, yard, H.floor) : 0, top = base + B.height, soilTop = top - B.soil;
    const inner = B.width - B.curb;
    const piece = (s, e, n0, depth, y0, y1) => ({ ...band(ax, n0, s, e, dir, depth), y0, y1 });
    /* Bó vỉa ngoài chạy suốt bồn, hai bó vỉa đầu lọt giữa mặt tường và bó vỉa ngoài. */
    const curbs = [piece(a, b, face + dir * inner, B.curb, base, top),
                   piece(a, a + B.curb, face, inner, base, top), piece(b - B.curb, b, face, inner, base, top)];
    const s0 = a + B.curb, s1 = b - B.curb, count = Math.max(1, Math.round((s1 - s0) / B.pitch));
    const soil = piece(s0, s1, face, inner, base, soilTop);
    /* Khóm hoa: tán vuông `clump` giữa lòng đất, gốc cắm xuống đất; chùm hoa nhỏ trùm trên đỉnh tán. */
    const mid = face + dir * inner / 2, step = (s1 - s0) / count;
    const square = (u, d, y0, y1) => ({ ...(ax === 'h' ? { x: u - d / 2, y: mid - d / 2 } : { x: mid - d / 2, y: u - d / 2 }),
                                        w: d, h: d, y0, y1 });
    const us = Array.from({ length: count }, (_, i) => s0 + step * (i + 0.5));
    const clumps = us.map(u => square(u, B.clump, soilTop - B.soil, soilTop + B.plant));
    const blooms = us.map(u => square(u, B.bloom, soilTop + B.plant - 0.07, soilTop + B.plant + 0.07));
    return { ...out, dir, face, rect, base, top, soilTop, room: yard?.[0], length: b - a, curbs, soil, clumps, blooms };
  });
}

/* ═══════════ ĐÈN ═══════════
   Hai danh sách khai, vì hai kiểu gắn khác hẳn nhau:

     `v.lights`:     [[mã, loại, x, y, mô tả], …]                  — đèn trần, đèn thả; (x, y) là tâm đèn
     `v.wallLights`: [[mã, loại, trục, vị trí, dọc, hướng, cao, mô tả], …] — đèn tường trên tường (trục, vị trí),
                     tâm ở toạ độ `dọc` theo trục tường, quay về phía `hướng` (±1), tâm cao `cao` tính từ sàn phía
                     nó quay vào. Tường tum cũng nhận đèn — khi đó sàn là mặt mái đi lại được.

   Loại tra ở LIGHT (lib/lot.js). Cao độ đèn trần không khai: nó áp mặt dưới vật che thấp nhất ngay trên — cái đó
   phải dựng xong mới biết (lib/massing.js). Ở đây chỉ soi phần khai: đèn trần có trần hay mái nào bên trên để gắn
   (không treo giữa trời, không treo trên lỗ thang — cao 6 m trên bậc, không ai thay bóng được), đèn tường nằm trên
   một bức tường có thật và không đè lỗ cửa. */
/* Phòng chứa một điểm: phòng kín trước — WC khách nằm lọt trong sân phơi — rồi phòng nhỏ hơn. */
function roomAtPoint(v, x, y){
  const holds = r => x >= r[2] - EPS && x <= r[2] + r[4] + EPS && y >= r[3] - EPS && y <= r[3] + r[5] + EPS;
  return v.rooms.filter(holds).sort((p, q) => isEnclosed(q) - isEnclosed(p) || p[4] * p[5] - q[4] * q[5])[0] || null;
}

/* Chỗ gắn một vật áp tường — đèn tường, bảng công tắc: trên tường (trục, vị trí), tâm ở `u` dọc trục, quay về phía
   `dir`, tâm cao `h` tính từ sàn phía nó quay vào; rộng `width` dọc tường, cao `height`, nhô `depth`.

   **Tường nhà trước, tường tum sau:** vách tây tum đứng thẳng trên tường trái nhà (cùng trục x = 0), vật khai trên
   trục ấy là vật trên tường nhà; tường tum chỉ nhận khi đoạn trục đó không có tường nhà nào. Trên tường tum sàn là mặt
   mái, quay vào trong tum thì thuộc buồng thang, quay ra thì không thuộc phòng nào. Không được đè lên lỗ cửa. */
function wallMount(v, { what, ax, pos, u, dir, h, width, height, depth }){
  if (!['h', 'v'].includes(ax) || Math.abs(dir) !== 1) return { errors: [`${what}: trục phải là h / v, hướng phải là ±1`] };
  const H = heightsOf(v), L = levels(H), walk = roofWalkTop(v), tum = tumOf(v), errors = [];
  const a = u - width / 2, b = u + width / 2;
  const wall = v.walls.find(([wax, wpos, wa, wb]) => wax === ax && Math.abs(wpos - pos) < EPS && wa <= a + EPS && wb >= b - EPS);
  const [s0, s1] = tum ? (ax === 'v' ? [tum.y0, tum.y1] : [tum.x0, tum.x1]) : [0, 0];
  const onTum = !wall && !!tum && (ax === 'v' ? [tum.x0, tum.x1] : [tum.y0, tum.y1]).some(e => Math.abs(e - pos) < EPS)
                && a >= s0 - EPS && b <= s1 + EPS;
  if (!onTum && !wall) return { errors: [`${what}: không có tường nào trục ${ax} ${pos} quanh ${u}`] };
  const t = onTum ? TUM.wall : wall[4], face = pos + dir * t / 2;
  const [px, py] = ax === 'h' ? [u, face + dir * 0.01] : [face + dir * 0.01, u];
  const inTum = onTum && px > tum.x0 && px < tum.x1 && py > tum.y0 && py < tum.y1;
  const room = onTum && !inTum ? null : roomAtPoint(v, px, py);
  if (!onTum && !room) return { errors: [`${what}: phía quay vào không có phòng hay sân nào`] };
  const base = onTum ? walk : floorOf(v, room, H.floor);
  const y0 = base + h - height / 2, y1 = base + h + height / 2;
  const openings = onTum
    ? tum.doors.filter(d => d.ax === ax && Math.abs(d.pos - pos) < EPS).map(d => ({ id: 'cửa tum', a: d.a, b: d.b, lo: walk, hi: walk + TUM.door }))
    : [...v.doors.filter(d => d[5] !== 'open').map(([oid, oax, opos, oa, ob]) => {
         const f = openingFloor(v, L, oax, opos, oa, ob);
         return { id: oid, ax: oax, pos: opos, a: oa, b: ob, lo: f, hi: f + (H.only[oid]?.door ?? H.door) };
       }),
       ...v.windows.map(([oid, oax, opos, oa, ob]) => {
         const f = openingFloor(v, L, oax, opos, oa, ob), only = H.only[oid] || {};
         return { id: oid, ax: oax, pos: opos, a: oa, b: ob, lo: f + (only.sill ?? H.sill), hi: f + (only.head ?? H.head) };
       })].filter(o => o.ax === ax && Math.abs(o.pos - pos) < EPS);
  for (const o of openings)
    if (overlap(a, b, o.a, o.b) > EPS && overlap(y0, y1, o.lo, o.hi) > EPS) errors.push(`${what} đè lên lỗ ${o.id}`);
  if (y0 < base + EPS) errors.push(`${what} cao ${h} m, chạm sàn`);
  const rect = band(ax, face, a, b, dir, depth);
  return { ax, pos, u, dir, h, face, a, b, y0, y1, rect, x: rect.x + rect.w / 2, y: rect.y + rect.h / 2,
           room: room?.[0] ?? null, roof: onTum, base, errors };
}

export function lightsOf(v){
  const H = heightsOf(v);
  const roomAt = (x, y) => roomAtPoint(v, x, y);
  const inRect = (c, x, y) => x >= c.x - EPS && x <= c.x + c.w + EPS && y >= c.y - EPS && y <= c.y + c.h + EPS;
  const covers = [
    ...v.rooms.filter(isEnclosed).map(([, , x, y, w, h]) => ({ x, y, w, h })),
    ...overhangsOf(v).filter(o => !o.error).map(o => o.rect),
    ...roofPanels(v).map(p => ({ x: p.x0, y: p.y0, w: p.x1 - p.x0, h: p.y1 - p.y0 })),
    ...gateRoofsOf(v).filter(r => !r.error).map(r => ({ x: r.rect.x, y: r.rect.z, w: r.rect.w, h: r.rect.d })),
  ];
  const seen = new Set(), out = [];
  const start = (id, type, want) => {
    const T = LIGHT[type], errors = [];
    if (seen.has(id)) errors.push(`Đèn ${id}: trùng mã`); else seen.add(id);
    if (!T) errors.push(`Đèn ${id}: không có loại '${type}'`);
    else if ((T.mount === 'wall') !== (want === 'wall'))
      errors.push(`Đèn ${id}: loại '${type}' là ${T.mount === 'wall' ? 'đèn tường, khai ở wallLights' : 'đèn trần, khai ở lights'}`);
    return { T, errors };
  };

  for (const [id, type, x, y, desc] of v.lights || []) {
    const { T, errors } = start(id, type, 'ceiling');
    if (errors.length) { out.push({ id, type, x, y, desc, errors }); continue; }
    const s = T.size / 2, rect = { x: x - s, y: y - s, w: T.size, h: T.size };
    const room = roomAt(x, y);
    if (!room) errors.push(`Đèn ${id} ở (${x}, ${y}) không nằm trong phòng hay sân nào`);
    const corners = [[rect.x, rect.y], [rect.x + rect.w, rect.y], [rect.x, rect.y + rect.h], [rect.x + rect.w, rect.y + rect.h]];
    if (!corners.every(([cx, cy]) => covers.some(c => inRect(c, cx, cy))))
      errors.push(`Đèn ${id} ở (${x}, ${y}) không có trần hay mái nào bên trên để gắn`);
    /* Đèn trong phòng gầm thang bám mặt dưới bậc, không treo lơ lửng trên lỗ. */
    if (!(room && isUnderStair(v, room)))
      for (const q of roofHoles(v))
        if (overlap(rect.x, rect.x + rect.w, q.x, q.x + q.w) > EPS && overlap(rect.y, rect.y + rect.h, q.y, q.y + q.h) > EPS)
          errors.push(`Đèn ${id} treo trên lỗ ${q.id} — quá cao để thay bóng`);
    out.push({ id, type, spec: T, mount: T.mount, x, y, desc, rect, room: room?.[0] ?? null,
               base: room ? floorOf(v, room, H.floor) : 0, errors });
  }

  for (const [id, type, ax, pos, u, dir, h, desc] of v.wallLights || []) {
    const { T, errors } = start(id, type, 'wall');
    if (errors.length) { out.push({ id, type, desc, errors }); continue; }
    const m = wallMount(v, { what: `Đèn ${id}`, ax, pos, u, dir, h, width: T.size, height: T.height, depth: T.depth });
    out.push({ id, type, spec: T, mount: 'wall', desc, ...m, errors: [...errors, ...m.errors] });
  }
  /* Theo số trong mã (Đ1, Đ2 … Đ10) cho bảng và đặc tả đọc theo thứ tự, dù khai ở hai danh sách. */
  const num = id => +String(id).replace(/\D/g, '') || 0;
  return out.sort((p, q) => num(p.id) - num(q.id));
}

/* ═══════════ CÔNG TẮC ═══════════
   `v.lightGroups`: [[mã, tên, [mã đèn]], …] — cụm đèn, mỗi hạt công tắc bật / tắt đúng một cụm.
   `v.switches`:    [[mã, trục, vị trí, dọc, hướng, cao, [mã cụm theo thứ tự hạt], mô tả], …] — bảng công tắc, gắn như
                    đèn tường (wallMount).

   Một cụm có hạt ở hai bảng là công tắc hai chiều. Soi: đèn nào cũng thuộc **đúng một** cụm (không cụm nào thì không
   bật được; hai cụm thì một bóng nối hai mạch), cụm nào cũng có ít nhất một hạt, bảng không quá SWITCH.maxGangs hạt
   và không có hai hạt cùng một cụm. Không khai gì thì trả về rỗng — kho đối chiếu không có. */
export function switchesOf(v){
  const errors = [], seen = new Set();
  const dup = (what, id) => { if (seen.has(id)) errors.push(`${what} ${id}: trùng mã`); seen.add(id); };
  const lampIds = lightsOf(v).map(l => l.id);
  const groups = (v.lightGroups || []).map(([id, name, lights = []]) => {
    dup('Cụm đèn', id);
    for (const l of lights) if (!lampIds.includes(l)) errors.push(`Cụm đèn ${id}: không có đèn ${l}`);
    if (!lights.length) errors.push(`Cụm đèn ${id} không có đèn nào`);
    return { id, name, lights, plates: [] };
  });
  const plates = (v.switches || []).map(([id, ax, pos, u, dir, h, gangs = [], desc]) => {
    dup('Bảng công tắc', id);
    const m = wallMount(v, { what: `Bảng công tắc ${id}`, ax, pos, u, dir, h,
                             width: SWITCH.width, height: SWITCH.height, depth: SWITCH.depth });
    if (!gangs.length) m.errors.push(`Bảng công tắc ${id} không có hạt nào`);
    if (gangs.length > SWITCH.maxGangs) m.errors.push(`Bảng công tắc ${id} có ${gangs.length} hạt, quá ${SWITCH.maxGangs} một mặt`);
    if (new Set(gangs).size !== gangs.length) m.errors.push(`Bảng công tắc ${id} có hai hạt cùng một cụm`);
    for (const g of new Set(gangs)) {
      const grp = groups.find(x => x.id === g);
      if (grp) grp.plates.push(id); else m.errors.push(`Bảng công tắc ${id}: không có cụm ${g}`);
    }
    return { id, desc, groups: gangs, ...m };
  });
  if (groups.length) {
    const owner = new Map();
    for (const g of groups) for (const l of g.lights)
      if (owner.has(l)) errors.push(`Đèn ${l} thuộc hai cụm ${owner.get(l)} và ${g.id}`); else owner.set(l, g.id);
    for (const l of lampIds) if (!owner.has(l)) errors.push(`Đèn ${l} không thuộc cụm nào — không có công tắc bật`);
    for (const g of groups) if (!g.plates.length) errors.push(`Cụm đèn ${g.id} ${g.name} không có hạt công tắc nào`);
  } else if (plates.length) errors.push('Có bảng công tắc mà không khai cụm đèn');
  return { groups, plates, errors };
}

/* Nội suy tuyến tính trên bảng [[x, y], …], ngoài hai đầu thì giữ giá trị đầu mút. */
...
/* Độ rọi chung từng phòng theo phương pháp quang thông (LIGHTING trong lib/lot.js). Mỗi phòng một hàng: số đèn,
   công suất, và nếu mặt bằng khai ngưỡng ở `v.lux` thì tính độ rọi. Cao trần lấy đúng cao trần thật của phòng —
   trần giả, trần tôn hay bản mái — vì nó quyết định chỉ số phòng. Sân và lối đi hở không tính độ rọi: không có
   tường trần để ánh sáng dội lại, phương pháp này không áp được. */
export function lightingOf(v){
  const H = heightsOf(v), L = levels(H), lamps = lightsOf(v).filter(l => !l.errors.length);
  const drops = dropCeilingsOf(v).filter(d => !d.error);
  return v.rooms.map(r => {
    const mine = lamps.filter(l => l.room === r[0]), general = mine.filter(l => l.mount !== 'wall');
    const row = { id: r[0], name: r[1], count: mine.length, watt: mine.reduce((s, l) => s + l.spec.watt, 0),
                  lumen: general.reduce((s, l) => s + l.spec.lumen, 0), target: v.lux?.[r[0]] };
    if (row.target == null) return row;
    const c = clearRect(v, r), floor = floorOf(v, r, H.floor), R = roofOver(v, r);
    const drop = drops.find(d => d.id === r[0]);
    const ceiling = drop ? drop.h : R ? R.ceiling - floor : L.ceiling - floor;
    const area = c.w * c.h, K = area / ((ceiling - LIGHTING.workPlane) * (c.w + c.h));
    const uf = interpolate(LIGHTING.uf, K), lux = row.lumen * uf * LIGHTING.maintenance / area;
    return { ...row, area, ceiling, K, uf, lux, ok: lux >= row.target - 1e-9 };
  });
}
...
export function roofInsulationOf(v){
  ...
  return { thickness: I.xps + I.paver, overhangs, errors,
           rooms: v.rooms.filter(r => isEnclosed(r) && !roofOver(v, r) && !isUnderStair(v, r)) };
}

/* ═══════════ DIỀM MÉP MÁI HIÊN ═══════════
   `v.overhangFascias`: [[trục, vị trí tường], …] — bản mái đổ ra ngoài tường nào có diềm (FASCIA), chọn theo tường nó
   bám như lớp chống nóng. Dải trước chạy suốt mép ngoài bản, kéo qua góc nếu đầu ấy có dải đầu; dải đầu chạy từ mặt
   tường ra mép ngoài. **Cạnh nào nằm trên ranh lô thì không ốp** — cả đầu bản (bản phủ tới mặt ngoài tường bao) lẫn mép
   ngoài: trần ban công sau đua đúng tới ranh `y = 30`, ốp ở đó là sang đất bên cạnh và đè lên đỉnh hai tường nâng. Đỉnh
   bằng mặt lát nếu bản có lớp chống nóng, không thì mặt bản. */
export function fasciasOf(v){
  const all = overhangsOf(v).filter(o => !o.error), ins = roofInsulationOf(v);
  const L = levels(heightsOf(v)), t = FASCIA.thickness;
  return (v.overhangFascias || []).map(([ax, pos]) => {
    const o = all.find(o => o.ax === ax && Math.abs(o.pos - pos) < EPS);
    if (!o) return { ax, pos, error: `Diềm mái hiên khai cho trục ${ax} ${pos}, nhưng không có mái đổ ra ngoài tường nào ở đó` };
    const covered = ins?.overhangs.some(q => q.ax === ax && Math.abs(q.pos - pos) < EPS);
    const top = covered ? roofWalkTop(v) : L.houseTop, y0 = top - FASCIA.height;
    const outer = o.face + o.dir * o.depth, n0 = Math.min(o.face, outer), n1 = Math.max(o.face, outer);
    const [edge, depthEdge] = ax === 'h' ? [LOT.w, LOT.d] : [LOT.d, LOT.w];
    const endA = o.a > EPS, endB = o.b < edge - EPS, front = outer > EPS && outer < depthEdge - EPS;
    const across = (u0, u1) => ax === 'h' ? { x: u0, y: n0, w: u1 - u0, h: n1 - n0 } : { x: n0, y: u0, w: n1 - n0, h: u1 - u0 };
    const pieces = [
      ...(front ? [{ ...band(ax, outer, o.a - (endA ? t : 0), o.b + (endB ? t : 0), o.dir, t), side: 'front' }] : []),
      ...(endA ? [{ ...across(o.a - t, o.a), side: 'a' }] : []),
      ...(endB ? [{ ...across(o.b, o.b + t), side: 'b' }] : []),
    ].map(p => ({ ...p, y0, y1: top }));
    return { ax, pos, top, y0, outer, overhang: o, pieces, error: null,
             length: (front ? o.b - o.a : 0) + (endA ? o.depth + t : 0) + (endB ? o.depth + t : 0) };
  });
}
