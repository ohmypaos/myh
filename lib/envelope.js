...
import { LOT, STEP, ROOF, ROOF_INSULATION, POST, BEAM, PURLIN, heightsOf } from './lot.js';
  ...
  for (const R of lightRoofs(v)) {
    ...
    for (const [ax, pos, a, b, dir] of edges) {
      ...
      for (const [s, e] of minusSpans([[a, b]], wallHolds(v, ax, pos, a, b, under))) {
        ...
        if (spans.some(q => q.ax === ax && Math.abs(q.pos - pos) < EPS && Math.abs(q.a - s) < EPS && Math.abs(q.b - e) < EPS)) continue;
        /* Dọc chiều dốc (hoặc trục v nếu mái bằng) là dầm chính, đi suốt. */
        const along = R.axis ? R.axis === (ax === 'h' ? 'x' : 'y') : ax === 'v';
        spans.push({ id: R.id, R, ax, pos, a: s, b: e, dir, along });
      }
    }
  }
  return spans;
...
export function beamsOf(v){
  ...
  /* Bề ngang dầm: giữa tim mép, hoặc lùi vào sau máng nếu mép ấy có máng treo tự do. */
  const spans = beamLines(v).map(s => {
    ...
    return { ...s, band: !g ? [s.pos - W / 2, s.pos + W / 2] : s.dir > 0 ? [g1, g1 + W] : [g0 - W, g0] };
  });
...
export function roofInsulationOf(v){
  ...
  return { thickness: I.xps + I.screed + I.tile, overhangs, errors,
