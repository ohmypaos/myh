...
/* `v.racks`: [[mã, trục, vị trí, từ, đến], …] — giàn phơi ngoài trời, cùng dạng khai với tuyến tường.
   Hai trụ ở hai đầu tuyến, các thanh phơi nối giữa (RACK). Chân đứng trên cốt sân của sân chứa nó —
   giàn phơi phải nằm trong một cái sân, không thì là dựng giữa nhà hoặc ngoài lô. */
export function racksOf(v){
  ...
  return (v.racks || []).map(([id, ax, pos, a, b]) => {
    ...
    const posts = ends.map(([x, y]) => ({ x: x - half, y: y - half, w: R.post, h: R.post,
                                          y0: base, y1: base + R.height }));
    /* Thanh phơi **cắm vào mặt trong hai trụ**, không chạy qua tim trụ — chạy qua tim là hai khối đặc chồng
       nhau, phép 10 bắt ngay. Tròn nên khai hộp bao kèm trục nằm, như thân bồn nước. */
    const [ba, bb] = [a + half, b - half];
    const bars = R.levels.map(lv => ({
      x: ax === 'h' ? ba : pos - R.bar / 2, w: ax === 'h' ? bb - ba : R.bar,
      y: ax === 'h' ? pos - R.bar / 2 : ba, h: ax === 'h' ? R.bar : bb - ba,
      y0: base + lv - R.bar / 2, y1: base + lv + R.bar / 2, along: ax === 'h' ? 'x' : 'z' }));
    if (R.levels.some(lv => lv + R.bar / 2 > R.height + EPS))
      errors.push(`Giàn phơi ${id}: có thanh phơi cao hơn đầu trụ`);
    return { id, ax, pos, a, b, room: yard?.[0], base, top: base + R.height,
             length: (b - a) * R.levels.length, posts, bars, errors };
  });
}
