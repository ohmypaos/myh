...
import { isEnclosed, floorOf, roomsAlong, openingFloor, stepsOf, overhangsOf, jointHalf, wallThickness,
         ...
         roofInsulationOf, roofHoles, stairsOf, tumOf, roofRailingsOf, tanksOf, racksOf, solidFencesOf, gateRoofsOf, gateDoorsOf,
         lightsOf, switchesOf, plantersOf, flowerBedsOf, treesOf } from './envelope.js';
...
/* Cánh cửa quay (1 cánh, bản lề `hinge` = đầu a / b) mở 90° về phía `open` (+1 = phía toạ độ lớn); cửa
   4 cánh thì hai cánh ngoài đóng, hai cánh giữa mở. `leavesOpen=false` đặt toàn bộ cánh vào đúng lỗ
  ...
  for (const [id, ax, pos, a, b, style, hinge, open] of v.doors) {
    ...
    if (!leavesOpen) {
      if (style === 'swing') closedLeaf(ua, ub);
        ...
        const q = (ub - ua) / 4;
        for (let i = 0; i < 4; i++) closedLeaf(ua + i * q, ua + (i + 1) * q);
      }
      continue;
    }
...
export function buildMassing(v, { doorsOpen = true } = {}){
  ...
  /* Đèn tra chỗ gắn trên khối đã gỡ chồng — tường, bản mái, mái nhẹ, mái cổng — nên dựng sau cùng. */
  const lamps = buildLamps(v, [...main, ...prisms]);
  const extra = [...insulation, ...st.boxes, ...tanks, ...racks.boxes, ...gateRoofs.boxes, ...gateDoors, ...buildGarden(v), ...buildFurniture(v, L), ...buildDoorLeaves(v, H, L, doorsOpen), ...lamps, ...buildSwitches(v)];
