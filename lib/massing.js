...
import { isEnclosed, floorOf, roomsAlong, openingFloor, stepsOf, overhangsOf, jointHalf, wallThickness,
         ...
         roofInsulationOf, roofHoles, stairsOf, tumOf, roofRailingsOf, tanksOf, racksOf, solidFencesOf, gateRoofsOf } from './envelope.js';
...
export function buildMassing(v){
  ...
  const st = buildStairs(v), tum = buildTum(v), roofRails = buildRoofRailings(v), tanks = buildTanks(v), racks = buildRacks(v), gateRoofs = buildGateRoofs(v);
  ...
  const extra = [...insulation, ...st.boxes, ...tanks, ...racks.boxes, ...gateRoofs.boxes, ...buildFurniture(v, L), ...buildDoorLeaves(v, H, L)];
