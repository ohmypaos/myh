...
import { isEnclosed, floorOf, roomsAlong, openingFloor, stepsOf, overhangsOf, jointHalf, wallThickness,
         ...
         roofInsulationOf, roofHoles, stairsOf, tumOf, roofRailingsOf, tanksOf } from './envelope.js';
...
export function buildMassing(v){
  ...
  const st = buildStairs(v), tum = buildTum(v), roofRails = buildRoofRailings(v), tanks = buildTanks(v);
  ...
  const extra = [...insulation, ...st.boxes, ...tanks, ...buildFurniture(v, L), ...buildDoorLeaves(v, H, L)];
