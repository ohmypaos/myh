...
import { isEnclosed, floorOf, roomsAlong, openingFloor, stepsOf, overhangsOf, jointHalf, wallThickness,
         ...
         roofInsulationOf, roofHoles, stairsOf, tumOf, roofRailingsOf, tanksOf, racksOf } from './envelope.js';
...
/* Giàn phơi: hai trụ hộp + các thanh phơi tròn (hộp bao kèm `along`, như thân bồn). */
const buildRacks = v => racksOf(v).filter(r => !r.errors.length).flatMap(r => [
  ...r.posts.map(q => ({ x: q.x, w: q.w, z: q.y, d: q.h, y0: q.y0, y1: q.y1, kind: 'rack', id: r.id })),
  ...r.bars.map(q => ({ x: q.x, w: q.w, z: q.y, d: q.h, y0: q.y0, y1: q.y1, kind: 'rackBar', id: r.id,
...
export function buildMassing(v){
  ...
  const extra = [...insulation, ...st.boxes, ...tanks, ...racks, ...buildFurniture(v, L), ...buildDoorLeaves(v, H, L)];
