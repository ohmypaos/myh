...
import { isEnclosed, floorOf, roomsAlong, openingFloor, stepsOf, overhangsOf, jointHalf, wallThickness,
         ...
         roofInsulationOf, roofHoles, stairsOf, tumOf, roofRailingsOf } from './envelope.js';
...
function buildStairs(v){
  ...
  for (const s of stairsOf(v)) {
    ...
    /* Dọc vế 2: tay vịn nghiêng song song đường mũi bậc, ba song đứng trên mỗi mặt bậc — khe chừng 7 cm. Song đứng
       trên mặt bậc, đỉnh chạm mặt dưới tay vịn ở mép thấp của song. */
    const nose = x => s.landingTop + s.rise * (1 + (x - xs) / s.tread);
    const yb = [nose(xs) + rail - hand, nose(xe) + rail - hand];
    prisms.push(tag({ x: xs, w: xe - xs, z: zc - hand / 2, d: hand, axis: 'x', yb, yt: yb.map(y => y + hand),
                      y0: yb[0], y1: yb[1] + hand }));
    for (const t of s.treads.filter(t => t.flight === 2))
      for (const f of [1 / 6, 1 / 2, 5 / 6]) {
        const cx = t.x + t.w * f;
        boxes.push(tag({ x: cx - bar / 2, w: bar, z: zc - bar / 2, d: bar, y0: t.top, y1: nose(cx - bar / 2) + rail - hand }));
      }
  }
  return { boxes, prisms };
}
...
export function buildMassing(v){
  ...
  const st = buildStairs(v), tum = buildTum(v), roofRails = buildRoofRailings(v);
  const prisms = [...w.prisms, ...s.prisms, ...st.prisms, ...tum.prisms];
  ...
  const top = Math.max(L.houseTop, ...prisms.map(p => p.y1), ...insulation.map(b => b.y1),
                       ...tum.boxes.map(b => b.y1), ...roofRails.map(b => b.y1));
  const extra = [...insulation, ...st.boxes, ...buildFurniture(v, L), ...buildDoorLeaves(v, H, L)];
