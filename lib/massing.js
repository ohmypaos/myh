...
import { isEnclosed, floorOf, roomsAlong, openingFloor, stepsOf, overhangsOf, jointHalf, wallThickness,
         ...
         roofInsulationOf, roofHoles, stairsOf, isUnderStair, stairCabinetOf, tumOf, roofRailingsOf, tanksOf, racksOf, solidFencesOf, gateRoofsOf, gateDoorsOf,
...
export function furnitureHeight(v, item){
  const [kind, , , w, h] = item;
  if (kind === 'stairCab') return Math.max(0, stairCabinetOf(v, item).height);   // cao theo gầm thang, suy ra
