...
import { isEnclosed, floorOf, roomsAlong, openingFloor, stepsOf, overhangsOf, jointHalf, wallThickness,
         levels, roofOver, roofCovering, roofPanels, gutters, downpipes, postsOf, beamsOf, purlinsOf, dropCeilingsOf, wallTopOf, clearRect } from './envelope.js';
...
function furnitureParts(v, item, room){
  ...
  const part = (x0, x1, z0, z1, y0, y1, surfaceKind) => ({ x0, x1, z0, z1, y0, y1, surfaceKind });
  ...
  /* Máy giặt/sấy là một khối đặt trên bệ sân cao 15 cm, rộng hơn thân máy 6 cm mỗi phía.
     Nước rửa sân vì thế không tràn thẳng vào thiết bị, còn bệ vẫn đủ thấp để bước lên. */
  if (kind === 'washRaised') {
    const rise = 0.15, pad = 0.06;
  ...
  /* Vòi nước âm tường: chỉ dựng đầu vòi nhô ra khỏi mặt trong tường, không có chân hay bệ từ
     sàn. Hình chữ nhật khai bắt đầu ở mặt tường và chiều ngắn là bề dày đầu vòi. */
  if (kind === 'tap') return [part(x, x + w, y, y + h, 0.95, 1.10)];
  ...
  (v.furn || []).forEach((item, i) => {
    ...
    for (const p of furnitureParts(v, item, room)) {
      ...
      boxes.push({ x: x0, w: x1 - x0, z: z0, d: z1 - z0, y0: f + p.y0, y1: f + p.y1,
                   kind: p.surfaceKind || 'furniture', id: item[0], item: i });
    }
  });
  return boxes;
}
...
export function buildMassing(v){
  ...
  const extra = [...buildFurniture(v, L), ...buildDoorLeaves(v, H, L)];
