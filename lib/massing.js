...
import { LOT, ROOF, RAILING, FURNITURE, DOOR_LEAF, heightsOf } from './lot.js';
import { isEnclosed, floorOf, roomsAlong, openingFloor, stepsOf, overhangsOf, jointHalf, wallThickness,
         levels, roofOver, roofCovering, roofPanels, gutters, downpipes, postsOf, dropCeilingsOf, wallTopOf, clearRect } from './envelope.js';
...
function buildSlabs(v, H, L){
  ...
  /* Cột đỡ mái nhẹ — từ cốt sân tới mặt dưới mái hoặc đáy máng (lib/envelope.js). Nằm trên tim tường rào
...
function furnitureParts(v, item, room){
  ...
  const part = (x0, x1, z0, z1, y0, y1) => ({ x0, x1, z0, z1, y0, y1 });
  ...
  (v.furn || []).forEach((item, i) => {
    ...
    for (const p of furnitureParts(v, item, room)) {
      ...
      boxes.push({ x: x0, w: x1 - x0, z: z0, d: z1 - z0, y0: f + p.y0, y1: f + p.y1,
                   kind: 'furniture', id: item[0], item: i });
    }
  });
  return boxes;
}
